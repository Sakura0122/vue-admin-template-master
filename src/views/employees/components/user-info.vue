  ...
  methods: {
    ...
    async saveUser() {
      ...
      if (staffRef.fileList && staffRef.fileList.length > 0) {
        this.userInfo.staffPhoto = staffRef.fileList[0].url
      } else {
        return this.$message.error('必须上传头像')
      }
      if (!staffRef.isAllUploadSuccess) {
        return this.$message.error('图片正在上传，请稍后再试')
      }
    ...
    async getPersonalDetail() {
      ...
      this.$refs.staffPhotoRef.fileList = [{ url: data.staffPhoto }]
      this.formData = data
    },
    async savePersonal() {
      ...
      if (!imgUrl) {
        return this.$message.error('必须上传照片')
      }
